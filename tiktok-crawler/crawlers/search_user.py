...
async def _crawl_profile_in_tab(context, semaphore, keyword, username, delay_range):
    """Crawl 1 profile trong tab riêng (concurrent-safe)"""
    async with semaphore:
        tab = await context.new_page()
        try:
            user_data = await asyncio.wait_for(
                crawl_profile(tab, keyword, username),
                timeout=30
            )
            logger.info(
            ...
            logger.warning(f"⏰ Timeout @{username} — skip")
            return None
    ...
    **kwargs,
    ...
    if deep_scan and context:
        ...
        for i in range(0, len(usernames), batch_size):
            ...
            logger.info(f"🚀 Concurrent batch {i // batch_size + 1} — {len(batch)} users")

            tasks = [
                _crawl_profile_in_tab(context, semaphore, keyword, u, delay_range)
                for u in batch
            ]
    ...
    # ===== SEQUENTIAL (light scan hoặc không có context) =====
    else:
        for idx, username in enumerate(usernames, 1):
            try:
                logger.info(f"🔄 Processing user {idx}/{len(usernames)}: @{username}")
                user_data = {
                    "keyword": keyword,
                    "tiktok_id": username,
                    "username": username,
