/**
 * CH Play Crawler - Frontend Logic
 */
...
async function searchApps() {
    ...
    const langMap = { vn: 'vi', us: 'en', jp: 'ja', kr: 'ko', gb: 'en', sg: 'en', th: 'th' };
    ...
    try {
        const resp = await fetch('/api/search', {
            ...
            body: JSON.stringify({ keyword, country, lang }),
        });
...
function renderAppList(apps) {
    ...
    grid.innerHTML = apps.map((app, idx) => `
        <div class="app-card" onclick="loadReviews(${idx})" style="animation-delay: ${idx * 0.03}s">
            <img class="app-icon" src="${app.icon}" alt="${escapeHtml(app.title)}" loading="lazy">
            <div class="app-info">
                <div class="app-name">${escapeHtml(app.title)}</div>
                <div class="app-developer">${escapeHtml(app.developer)}</div>
                <div class="app-meta">
                    <span class="app-rating">⭐ ${app.score || 'N/A'}</span>
                    <span>${formatNumber(app.ratings)} đánh giá</span>
                    ${app.genre ? `<span class="app-genre">${escapeHtml(app.genre)}</span>` : ''}
                    ${app.installs ? `<span class="app-installs">📥 ${app.installs}</span>` : ''}
                </div>
            </div>
        </div>
    `).join('');
...
async function loadReviews(index) {
    ...
    appInfo.innerHTML = `
        <img src="${app.icon}" alt="${escapeHtml(app.title)}">
        <div class="info-text">
...
function renderRatingSummary(data) {
    ...
    barsEl.innerHTML = [5, 4, 3, 2, 1].map(rating => {
        const count = data.rating_counts[String(rating)] || 0;
        return `
...
function switchTab(rating) {
    ...
    if (rating === 'all') {
        for (let r = 5; r >= 1; r--) {
            reviews = reviews.concat(currentReviews.reviews_by_rating[String(r)] || []);
        }
    } else {
        reviews = currentReviews.reviews_by_rating[String(rating)] || [];
    }
        ...
        if (review.replyContent) {
            replyHtml = `
                <div class="review-reply">
                    <div class="review-reply-label">📩 Phản hồi từ nhà phát triển</div>
                    <div class="review-reply-content">${escapeHtml(review.replyContent)}</div>
                    ${review.replyDate ? `<div class="review-reply-date">${formatDate(review.replyDate)}</div>` : ''}
                </div>
            `;
        }
        return `
            <div class="review-card" style="animation-delay: ${Math.min(idx * 0.03, 0.5)}s">
                ...
                ${review.thumbsUpCount > 0 ? `<span class="review-meta">👍 ${review.thumbsUpCount}</span>` : ''}
                ${review.reviewCreatedVersion ? `<span class="review-meta"> 📱 v${escapeHtml(review.reviewCreatedVersion)}</span>` : ''}
                ${replyHtml}
            </div>
        `;
    }).join('');
...
async function exportCSV() {
    ...
    try {
        ...
        a.href = url;
        a.download = `reviews_${currentSelectedApp.title.replace(/[^a-zA-Z0-9]/g, '_')}.csv`;
